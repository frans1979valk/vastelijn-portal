...
Plak dit als 1 bericht in Claude Code.

```txt
Je werkt in repo "vastelijn-portal". Doel: Headwind MDM integratie toevoegen zodat de QR/payload per device een echte Android Enterprise provisioning QR wordt en de juiste policy/app wordt toegepast.
...
6) Update de frontend deviceView: toon een echte QR-code weergave.
   - Genereer QR als SVG client-side uit de payload string.
...
- Geef daarna een korte deployment checklist.
```
